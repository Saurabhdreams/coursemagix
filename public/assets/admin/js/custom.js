...
$(function ($) {
  ...
  ******************************************************/
  ...
  $("#submitAboutUs").on('click', function (e) {
    $(e.target).attr('disabled', true);
    $(".request-loader").addClass("show");
    let ajaxForm = document.getElementById('aboutUs');
    let fd = new FormData(ajaxForm);
    let url = $("#aboutUs").attr('action');
    let method = $("#aboutUs").attr('method');

    if ($("#aboutUs .summernote").length > 0) {
      $("#aboutUs .summernote").each(function (i) {
        let content = $(this).summernote('isEmpty') ? '' : $(this).summernote('code');
        fd.delete($(this).attr('name'));
        fd.append($(this).attr('name'), content);
      });
