...
use App\Http\Requests\CountInformationRequestUpdate;
...
class FunFactController extends Controller
{
  ...
  public function store(CountInformationRequest $request)
  {
    ...
    if($request->hasFile('image')){
      $imageName = Uploader::upload_picture(Constant::WEBSITE_FUN_FACT_SECTION_IMAGE, $request->file('image'));
    }
      CountInformation::create($request->except('user_id','language_id','image')+[
        'user_id' => Auth::guard('web')->user()->id,
  ...
  public function update(CountInformationRequestUpdate $request)
  {
    $CountInformation = CountInformation::query()->where('user_id', Auth::guard('web')->user()->id)->find($request->id);
    if ($request->hasFile('image')) {
      $imageName = Uploader::update_picture(Constant::WEBSITE_FUN_FACT_SECTION_IMAGE, $request->file('image'),$CountInformation->image);
    }
    $info = CountInformation::where('user_id', Auth::guard('web')->user()->id)->find($request->id);
    $info->update($request->except('image') + [
      'image' => $request->hasFile('image') ? $imageName : $CountInformation->image
  ]);
    $request->session()->flash('success', 'Information updated successfully!');
    return "success";
  }
