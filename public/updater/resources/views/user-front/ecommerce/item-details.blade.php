...
@section('meta-description', !empty($userSeo) ? $userSeo->item_details_meta_description : '')
@section('meta-keywords', !empty($userSeo) ? $userSeo->item_details_meta_keywords : '')
...
@section('content')
    <!------=== breadcum---------->
    @includeIf('user-front.common.partials.breadcrumb', ['breadcrumb' => $bgImg->breadcrumb, 'title' => $pageHeading->shop_details_page_title ?? 'Item Details' ])

    <!--====== Shop Section Start ======-->
    @php
        $isFlash = App\Http\Helpers\CheckFlashItem::isFlashItem($ad_details->item_id);
        $variations = App\Models\Ecommerce\UserItemVariation::where('item_id', $ad_details->item_id)
                    ...
                    <div class="col-lg-6">
                        <div class="product-summary">
                            ...
                            @if ($ad_details->item->sku)
                                <div class="sku mb-2">
                                    {{ __('SKU') }} : <strong>{{ $ad_details->item->sku }}</strong>
                                </div>
                            @endif
                                ...
                                <form action="#">
                                    ...
                                    <div class="add-to-cart-btn">
                                        <a href="javascript:void(0)" class="wish add-to-wish cursor-pointer"
                                            @if (!empty($myWishlist) && in_array($ad_details->item_id, $myWishlist)) title="{{ $keywords['Remove_From_Wishlist'] ?? __('Remove From wishlist') }}" @else title="{{ $keywords['Add_To_Wishlist'] ?? __('Add to wishlist') }}" @endif
                                            data-item_id="{{ $ad_details->item_id }}"
                                ...
                                @if (!empty($userShopSetting) && $userShopSetting->item_rating_system)
                                    <div class="tab-pane fade" id="tab-review" role="tabpanel">
                                                    ...
                                                    @foreach ($reviews as $review)
                                                        <li class="single-review">
                                                            <div class="review-thumb">
                                                                ...
                                                                <img data-src="{{ is_null($review->customer->image) ? asset('assets/front/img/noimage.jpg') : \App\Http\Helpers\Uploader::getImageUrl(Constant::WEBSITE_TENANT_CUSTOMER_IMAGE . '/' . $userBs->id, $review->customer->image, $userBs) }}"
                                                                    class="lazy" alt="Image">
                                                            </div>
                                        ...
                                        @if (Auth::guard('customer')->user())
                                            @if (App\Models\Ecommerce\UserOrderItem::where('customer_id', Auth::guard('customer')->user()->id)->where('item_id', $ad_details->item_id)->exists())
    ...
    @include('user-front.ecommerce.partials.variation-modal')
