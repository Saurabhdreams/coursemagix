...
@section('content')
    <!------=== breadcum---------->
      @includeIf('user-front.common.partials.breadcrumb', ['breadcrumb' => $bgImg->breadcrumb, 'title' => $pageHeading->cart_page_title ?? 'cart' ])
                    ...
                    <div id="refreshDiv">
                        <div class="cart-block">
                            <ul class="total-item-info">
                                ...
                                @endphp
                                <li><strong>{{ $keywords['Total_Items'] ?? 'Total Items' }}:</strong> <strong
                                        class="cart-item-view">{{ $cart ? $countitem : 0 }}</strong></li>
                                <li><strong>{{ $keywords['Cart_Total'] ?? 'Cart Total' }} :</strong> <strong
                                        class="cart-total-view">{{ $userBs->base_currency_symbol_position == 'left' ? $userBs->base_currency_symbol : '' }}{{ $cartTotal }}{{ $userBs->base_currency_symbol_position == 'right' ? $userBs->base_currency_symbol : '' }}</strong>
                                </li>
                            </ul>
                                                    ...
                                                    <td class="price cart_price">
                                                        <div class="variation-content">
                                                            <p>
                                                                ...
                                                                {{ $userBs->base_currency_symbol_position == 'left' ? $userBs->base_currency_symbol : '' }}<span>{{ $item['variations'] ? $item['total'] - $v_total * $item['qty'] : $item['product_price'] * $item['qty'] }}</span>{{ $userBs->base_currency_symbol_position == 'right' ? $userBs->base_currency_symbol : '' }}
                                                            </p>
                                                            @if (!empty($item['variations']))
                                                                <p>
                                                                    <strong>{{ __('Variation') }}:</strong>
                                                                    {{ $userBs->base_currency_symbol_position == 'left' ? $userBs->base_currency_symbol : '' }}<span>{{ $v_total * $item['qty'] }}</span>{{ $userBs->base_currency_symbol_position == 'right' ? $userBs->base_currency_symbol : '' }}
                                                                </p>
                                                            @endif
                                                        </div>
                                                    </td>
                                                    <td class="sub-total">
                                                        {{ $userBs->base_currency_symbol_position == 'left' ? $userBs->base_currency_symbol : '' }}{{ $item['total'] }}{{ $userBs->base_currency_symbol_position == 'right' ? $userBs->base_currency_symbol : '' }}
                                                    </td>
                                                    <td>
                                                        <div class="remove">
                                                            <div class="checkbox">
