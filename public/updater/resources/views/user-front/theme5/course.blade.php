        ...
        <figure class="product-img">
            <a href="{{ route('front.user.course.details', [getParam(), 'slug' => $course->slug]) }}" title="Image"
                ...
                <img class="lazyload"
                    ...
                    alt="{{ strlen($course->title) > 45 ? mb_substr($course->title, 0, 45, 'UTF-8') . '...' : $course->title }}">
            </a>
            <div class="hover-show">
                <a href="{{ route('front.user.course.details', [getParam(), 'slug' => $course->slug]) }}"
                    class="btn btn-md btn-primary radius-sm" title="{{ $keywords['enroll_now'] ?? __('Enroll Now') }}"
        ...
        <div class="product-details">
            <div class="p-3">
                ...
                <h6 class="product-title lc-2 mb-0">
                    <a href="{{ route('front.user.course.details', [getParam(), 'slug' => $course->slug]) }}"
                        target="_self"
                        title="{{ strlen($course->title) > 45 ? mb_substr($course->title, 0, 45, 'UTF-8') . '...' : $course->title }}">
