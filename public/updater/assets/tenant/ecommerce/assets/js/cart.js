    ...
    $.get(
        ...
        function (res) {
            ...
            if (res.message) {
                ...
                var x = $("#cartIconWrapper").load(location.href + " #cartIconWrapper",function(){
                    $(".request-loader").hide();
        ...
        if (variations != null) {
            ...
            for (var key in variations) {
                ...
                for (let i = 0; i < options_name.length; i++) {
                    variants += `<input class="voptions" ${
                        i == 0 ? "checked" : ""
                    } data-key="${i}" data-option="${
                        variations[key].variant_name
                    }" data-name="${options_name[i]}" data-price="${parseFloat(
                        options_price[i] -
                            (flash_percentage * options_price[i]) / 100
                    ).toFixed(2)}" data-stock="${
                        options_stock[i]
                    }" type="radio" id="${variations[key].id}${
                        options_name[i]
                    }" name="${variations[key].variant_name}"
                                    value="">
                                <label for="${variations[key].id}${
                        options_name[i]
                    }">${options_name[i]} +
                                ${textPosition == "left" ? currSymbol : ""}
                                ${parseFloat(
                                    options_price[i] -
                                        (flash_percentage * options_price[i]) /
                                            100
                                ).toFixed(2)}
            ...
            $(".modal-cart-link").attr("data-href", url);
        } else {
            $(".request-loader").show();
            let $this = $(this);
            let product_price = $this.attr("data-current_price");
            let qty =
                $("#detailsQuantity").length > 0
                    ? $("#detailsQuantity").val()
                    : 1;
    ...
    $(document).on("click", ".modal-cart-link", function () {
        ...
        if ($voptions.length > 0) {
            $voptions.each(function () {
                ...
                if (stock < currval) {
                    stErrMsg.push(
                        v_name + " : " + v_op_name + " ; stock unavailable"
                    );
                    stErr = 1;
                } else {
                    $input.val(currval);
    ...
    $(document).on("click", "#updateCart", function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.message) {
                    $("#cartIconWrapper").load(location.href + " #cartIconWrapper",function(){
