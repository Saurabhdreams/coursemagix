                        ...
                        <div class="course-video-nav mt-15">
                            ...
                            @if (Auth::guard('customer')->check() &&
                                    $lesson->lesson_complete()->where('customer_id', Auth::guard('customer')->user()->id)->count() > 0) lesson-complete @endif"
                                                        id="lesson-{{ $lesson->id }}">
                                                        <span>{{ $lesson->title }}
                                                            {{ '(' . $lessonDuration->format('i') . ':' }}{{ $lessonDuration->format('s') . ')' }}</span>
                                                    </a>
                                                </li>
                                            @endforeach
                                        @endif
                                    ...
                                    @case('video')
                                        <div class="video-box">
                                            <video id="videoId{{ $lessonContent->id }}" class="video-js vjs-16-9" controls
                                                ...
                                                poster="{{ \App\Http\Helpers\Uploader::getImageUrl(Constant::WEBSITE_LESSON_CONTENT_VIDEO_PREVIEW, $lessonContent->video_preview, $userBs, 'assets/tenant/image/static/default_video_preview.jpeg') }}">
                                                <source
                                                    src="{{ \App\Http\Helpers\Uploader::getImageUrl(Constant::WEBSITE_LESSON_CONTENT_VIDEO, $lessonContent->video_unique_name, $userBs) }}"
                                                    type="video/mp4">
                                            </video>
                                        </div>
                                    @break
