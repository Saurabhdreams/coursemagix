        ...
        <div class="container">
            <div class="d-flex flex-wrap justify-content-between gap-15 align-items-center">
                ...
                <div class="social-link style-2 size-md">
                    ...
                    <a class="rounded-pill" href="{{ $socialMediaInfo->url }}" target="_blank" title=""><i class="{{ $socialMediaInfo->icon }}"></i></a>
                    @endforeach
                </div>
                @endif
            </div>
        ...
        <div class="container">
            <nav class="navbar navbar-expand-lg">
                ...
                @if (!is_null($websiteInfo->logo))
                    <a class="navbar-brand" href="{{ route('front.user.detail.view', getParam()) }}" target="_self" title="">
                ...
                <div class="collapse navbar-collapse">
                    <ul id="mainMenu" class="navbar-nav mobile-item mx-auto">
                      ...
                      @else
                        <li class="nav-item">
                            <a href="{{$href}}" class="nav-link toggle"> {{$link["text"]}} <i class="fal fa-plus"></i></a>
                            <ul class="menu-dropdown">
                               ...
                               @endphp
                                <li class="nav-item">
                                    <a class="nav-link" href="{{$l2Href}}">{{$level2["text"]}}</a>
                                </li>
                                @endforeach
                            </ul>
                        </li>
                    ...
                    <div class="item">
                        <div class="language">
                            ...
                            <select class="niceselect" name="lang_code"  onchange="this.form.submit()">
                                ...
                                <option value="{{ $languageInfo->code }}" {{ $languageInfo->code == $currentLanguageInfo->code ? 'selected' : '' }}>{{ $languageInfo->name }}</option>
                                @endforeach
                            </select>
                        </form>
                        </div>
                    ...
                    <div class="item">
                     ...
                     @endguest
                      ...
                      @php $authUserInfo = Auth::guard('customer')->user(); @endphp
                        <a href="{{route('customer.dashboard', getParam())}}" class="btn-icon-text" target="_self" title="{{$keywords['dashboard'] ?? __('Dashboard') }}">
                            <i class="fal fa-user"></i> {{$keywords['dashboard'] ?? __('Dashboard') }}
                        </a>
                      @endauth
                    </div>
                </div>
