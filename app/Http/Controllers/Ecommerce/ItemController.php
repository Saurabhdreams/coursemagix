...
use App\Models\Ecommerce\UserItemImage;
...
class ItemController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['lang'] = $lang;
        // dd($data['items']);
        return view('user.ecommerce.item.index', $data);
    }
    ...
    public function type(Request $request)
    {
        ...
        return view('user.ecommerce.item.type', $data);
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('user.ecommerce.item.create', $data);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('user.ecommerce.item.edit', $data);
    }
    ...
    public function status(Request $request)
    {
        ...
        if ($request->status == 1) {
            ...
            Session::flash('success', 'Item Deactive successfully!');
        }
        return back();
    }
    ...
    //                 if (!empty($request[$value->code . '_options1' . '_' .  $v_helper])) {
    //                     foreach ($request[$value->code . '_options1' . '_' .  $v_helper] as  $option) {
    //                         if (empty($option)) {
    ...
    //                     'variant_name' => $request[$value->code . '_variation_' . $v_helper],
    //                     'option_name' => json_encode($request[$value->code . '_options1' . '_' .  $v_helper]),
    //                     'option_price' => json_encode($request['options2' . '_' .  $v_helper]),
    //                     'option_stock' => json_encode($request['options3' . '_' .  $v_helper]),
    //                     'indx' =>  $v_helper
    ...
    public function variations(UserItem $useritem)
    {
        ...
        return view('user.ecommerce.item.variation', $data);
    }

    public function settings()
    {
        $data['shopsettings'] = UserShopSetting::where('user_id', Auth::guard('web')->user()->id)->first();
        return view('user.ecommerce.item.settings', $data);
    }
