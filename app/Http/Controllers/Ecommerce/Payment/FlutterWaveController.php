...
class FlutterWaveController extends Controller
{
    ...
    public function paymentProcess(Request $request, $_amount, $_email, $_item_number, $_successUrl, $_cancelUrl, $bex)
    {
        ...
        if ($err) {
            ...
            return redirect($cancel_url. '?message='.$err)->with('error', 'Curl returned error: ' . $err);
        }

        $transaction = json_decode($response);

        if($transaction->status == 'error'){
            return redirect($cancel_url . '?message='.$transaction->message)->with('error', 'API returned error: ' . $transaction->message);
        }
    ...
    public function successPayment(Request $request)
    {
        ...
        if (isset($request['txref'])) {
            ...
            if ($resp['status'] == 'error') {
                ...
                return redirect($cancel_url .'?message='.$message);
            }
