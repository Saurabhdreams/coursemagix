...
class FrontendController extends Controller
{
    ...
    {
        $rules = [
            'email' => 'required|email|unique:subscribers|regex:/^[a-z0-9._%+-]{1,50}@[a-z0-9.-]{1,50}\.[a-z]{2,}$/i'
        ];
    ...
    {
{
    ...
    public function lazyload(Request $request,$getuser,$viewpage,$part=null)
    {
        $user = getUser();
        $data['user'] = $user;
        ...
        if($viewpage == 'index' && $part == 1){
            ...
            ->select('user_courses.id', 'user_courses.thumbnail_image', 'user_courses.lessions', 'user_courses.pricing_type', 'user_courses.previous_price', 'user_courses.current_price', 'user_courses.average_rating', 'user_courses.duration', 'user_course_informations.title', 'user_course_informations.slug', 'user_course_categories.name as categoryName', 'user_instructors.image as instructorImage', 'user_instructors.name as instructorName','user_course_categories.slug as categorySlug','user_course_informations.id as course_information_id')
            ->orderByDesc('user_courses.id')
            ->get();

            $allCourses->map(function ($course) use ($user) {
                $course['moduleCount'] = Module::query()
                    ->where('user_id', $user->id)
                    ->where('course_information_id', '=', $course->course_information_id)
                    ->where('status', '=', 'published')
                    ->count();
            });
            $queryResult['allCourses'] =  $allCourses;


            $page = View('user-front.theme4.lazyload-part1',$queryResult)->render();
    ...
    public function userDetailView($domain)
    {
        ...
        // if the product remains active in cart
        if(Session::has('cart')){
            $carts = Session::get('cart');
            foreach($carts as $key => $cart){
                $product = UserItem::where('id',$cart['id'])->where('status',true)->first();
   ...
   //theme version 4,5,6
        if ($themeInfo->theme_version == 4 || $themeInfo->theme_version == 5 || $themeInfo->theme_version == 6) {
            ...
            $queryResult['allCourses'] =  $allCourses;
            $aboutSectionPoints = UserAboutUsPoint::where('user_id', $user->id)
                ->where('lang_id', $language->id)->get();
            $queryResult['aboutSectionPoints'] = $aboutSectionPoints;
        }
        ...
        } else if ($themeInfo->theme_version == 4) {
            return view('user-front.theme4.index', $queryResult);
        } else if ($themeInfo->theme_version == 5) {
            return view('user-front.theme5.index', $queryResult);
        } else if ($themeInfo->theme_version == 6) {
            return view('user-front.theme6.index', $queryResult);
        } else {
            return view('user-front.theme1.index', $queryResult);
        }
